...
class Product_ctrl
{
    ...
    public function trash_list($req = null)
    {
        ...
        $total_page = $this->product_list(ord: "DESC", limit: 10000, active: 0);
        $tp = count($total_page);
        if ($tp %  $data_limit == 0) {
            $tp = $tp / $data_limit;
        } else {
    ...
    public function edit($req = null)
    {
        ...
        $context = (object) array(
            ...
            'data' => (object) array(
                'req' => obj($req),
                'product_detail' => $this->product_detail($req->id),
    ...
    public function save($req = null)
    {
        ...
        if (isset($request->title)) {
            ...
            if (intval($postid)) {
                ...
                echo js_alert('Product created');
                echo go_to(route('productList'));
            } else {
                echo js_alert('Product not created');
                return false;
            }
        }
    }
    public function toggle_trending($req = null)
    {
        ...
        if (isset($request->content_id) && isset($request->action) && ($request->action == 'is_trending' || $request->action == 'is_featured')) {
            ...
            if (count($trending_post) > 0) {
                ...
                $res['msg'] = 'success';
                $res['data'] = "Product marked as $request->action";
            }
    ...
    public function update($req = null)
    {
        ...
        if (isset($request->title)) {
            ...
            try {
                ...
                echo js_alert('Product not updated, check slug or content data');
                exit;
            }
        }
    }
